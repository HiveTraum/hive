...
//	defer ctrl.Finish()
//	store := mocks.NewMockStoreInterface(ctrl)
//	backend := backends.JWTAuthenticationBackend{store: store}
//	loginController := AuthenticationController{backends: map[string]infrastructure.AuthenticationBackend{"Bearer": backend}}
//
//	userID := uuid.NewV4()
//	secret := &models.Secret{
//		Id:      uuid.NewV4(),
...
//	status, payload, ctx := loginController.Login(ctx, encodedToken)
//	require.Equal(t, enums.Ok, status)
...
//	t.Parallel()
//	ctx := context.Background()
//	controller := AuthenticationController{backends: map[string]infrastructure.AuthenticationBackend{"Bearer": backends.JWTAuthenticationBackend{}}}
//
...
//	store := mocks.NewMockStoreInterface(ctrl)
//	backend := backends.JWTAuthenticationBackend{store: store}
//	controller := AuthenticationController{backends: map[string]infrastructure.AuthenticationBackend{"Bearer": backend}}
//
//	userID := uuid.NewV4()
//	secret := &models.Secret{
//		Id:      uuid.NewV4(),
